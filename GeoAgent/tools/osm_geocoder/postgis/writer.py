...
def create_writer_chain(llm: BaseLanguageModel): # Erstellt eine RunnableSequence, die mit .invoke({"query":str) aufgerufen werden kann und einen Plan zum Schreiben eines Scripts auf Basis einer Aufgabenbeschriebung und Infos über GeoDataFrames zurückliefert
    writer_prompt = ChatPromptTemplate.from_template("""You are an experienced writer of SQL scripts for the analysis of OpenStreetMap (OSM) data and/or geocoding in PostGIS. Write a SQL script for the povided query based on the information about the database. Only return the executable SQL code.
    ...
    **Database information:**                                           
    ...
    Return only the SQL script that can be executed directly.
    Now, start writing for the following query!  
    Query: {formalized_query}""")


    writer_chain = writer_prompt | llm | StrOutputParser() # RunnableSequence
